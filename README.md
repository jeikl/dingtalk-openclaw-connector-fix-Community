...
对官方薄弱的图片发送机制做了**毫无死角**的补齐与修复，支持混合图片发送，多条路径均可正常渲染：

| 发送方式 | 能力 |
|----------|------|
| **普通对话回复** | Agent 用 markdown 语法 `![注释](…)` 嵌图，定稿前自动上传为 mediaId |
| **message 工具 `media` / mediaUrl** | 机器人主动触发，直发图片（本地 / 内网 / 公网） |
| **message 工具 markdown 正文** | 正文里嵌 `![](…)`（含本地、`file://`、直链），与文字同条发出 |
| **文图策略可配** | 默认文图**分开**；`messageImageMd: true` 时多图+文字可**合并**一条 markdown |
...
- 代码块 / 行内 code 内的路径**不会误上传**（参数说明原文保留）
- 远程图会先下载再上传；本地图上传失败会 `/tmp` 重试
- 图 + 下载链接可同泡：`![]` 转 mediaId，下载 URL 仍保留原链

### AI Card 模板

- 支持自定义流式卡模板；**不填 `cardTemplateId` 时默认**社区增强模板 `0d2c84b3-12c1-473b-b14a-f329a7a102cd.schema`（含复制按钮等）。
