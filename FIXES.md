...
## v0.8.21（2026-06-29 · 社区维护版）

> 基于官方 v0.8.21 拉取本仓库，社区维护版统合了 fix3 ~ fix19 的全部修复 + 本版独有的 6 项核心增强。本节记录本版（前缀 `0.8.21-fix20`）的修复/增强内容；早期 fix 条目（fix1 ~ fix6）参见下文历史段。
...
新增 **答案卡模式（`answerCard`，默认开启）**：最终答案 token 超过 `answerActToken`（默认 600）时，原流式卡定格显示 "✅ 思考完成"，另投一张**静态答案卡**承载完整回复。短答案（≤ 阈值）仍在原流式卡定稿，不多开卡。模板 ID / 阈值可配：`answerCardTemplateId` / `answerActToken`。
...
- `channels.dingtalk-connector.answerActToken` —— 触发阈值（token），默认 `600`
