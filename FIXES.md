...
### 🐛 修复 message 工具发卡 content 为空（reply-dispatcher 路径正常）

**问题描述**

Agent 调用 `message` 工具主动发送消息时，发送出来的 AI Card **content 字段完全是空白的**（钉钉侧显示一条无文字的卡片气泡，仅有"Generated by AI"标签）。同会话中对话回复的流式 AI Card 渲染正常，工具调用进度也正常显示。

**根因**

`finishAICard` 在 v0.8.21-fix19 被简化：删掉了对 `streamAICard` 的调用，改成直接 PUT `/card/instances` 设 FINISHED。注释里写"FINISHED 经 /card/instances 是全量更新，无需先进 INPUTING"——但这只对**已经过流式**的卡片成立。

钉钉 AI Card 的合法状态机是 `PROCESSING → INPUTING → FINISHED`。两条 finish 路径走向不同：

| 路径 | 调用链 | `inputingStarted` | 直接 PUT FINISHED 结果 |
|---|---|---|---|
| 对话回复 | `onPartialReply` → `streamAICard`（多次）→ `finishAICard` | `true`（已切换过 INPUTING） | ✅ content 正常渲染 |
| message 工具 | `createAICardForTarget` → **直接** `finishAICard` | `false`（新建卡未流式） | ❌ 钉钉侧不渲染 content（卡片空白） |

message 工具经 `outbound.sendText` → `sendTextToDingTalk` → `sendProactive` → `sendProactiveInternal` → `createAICardForTarget` 拿到一张**新建**卡片，`inputingStarted=false`，紧接着调 `finishAICard` 直接 PUT FINISHED —— 钉钉收到从未经过 INPUTING 的 FINISHED 请求，不渲染 content。
...
仅在 `inputingStarted=false`（message 工具的新建卡）时先调一次 `streamAICard`，让卡片走完 INPUTING 状态切换 + content 写入；再 PUT FINISHED。`finished=false` 触发的是 line 401-460 的 INPUTING 块 + line 463-471 的 streaming 块（`isFinalize=false`），**不会触发"假流式回放"**。

reply-dispatcher 路径（`inputingStarted=true`）不进此分支，行为完全不变 —— a5f120b 的"无假流式回放"优化保留。

**影响**

- ✅ message 工具发的 AI Card 现在能正常显示文本内容
- ✅ reply-dispatcher 路径不受影响（流式照常、定稿照常、无假流式回放）
- ✅ 答案卡（answerCard 模式）路径不受影响
- ✅ 降级到普通消息的路径不受影响
